...
def get_neighbor(protbb, neighbor: int = 32, noise_level=0.0, train=False, mask=True):
    ...
    nodes = torch.cat((F.one_hot(seq_toks.long(), 22), bb_ang),dim=-1).transpose(1, 0) # 32,277,28
    edge = torch.cat((dis, pos_num.unsqueeze(-1), chain_id.unsqueeze(-1)), dim=-1) # 277,32,28

    return nodes, edge.transpose(1,0), protbb.seq.squeeze(-1).long(),indices
